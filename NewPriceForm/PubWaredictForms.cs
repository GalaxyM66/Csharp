...
{
    public partial class PubWaredictForms : Form
    {
        APDao_GoodsClientPrice dao = new APDao_GoodsClientPrice();

        private SortableBindingList<SelPubWaredicts> PubWaredictList = new SortableBindingList<SelPubWaredicts>();
        public SelPubWaredicts waredictinfo = new SelPubWaredicts();
        public PubWaredictForms()
        {
            InitializeComponent();
            dgvBill.AutoGenerateColumns = false;
        }

        private void PubWaredictForms_Load(object sender, EventArgs e)
        {
            this.Text = "查询商品";
            txtGoodsCode.Focus();
        }
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (txtGoodsCode.Text.Length >= 3 || txtGoodsName.Text.Length >= 3)
            {
                ...
                PubWaredictList = dao.GetPubWaredictAllInfoLists(sqlkeydict);
        ...
        private void dgvBill_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex < 1)//第一列才触发事件
                return;
            waredictinfo = (SelPubWaredicts)(row[0].Cells[0].Value);
